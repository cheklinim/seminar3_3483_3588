...
PullRequest - это запрос на внесение изменений в чужой удаленный репозиторий. Чтобы сделать изменения и предложить их нужно провести следующие действия:

1. Сделать Fork необходимого репозитория (кнопка находится на его странице)
2. Cоздать пустую папку на своем компьютере, открыть ее в VS Code.
3. Загрузить "клон" чужого репозитория посредством команды: 

       git clone <ссылка на клон репозитория на ВАШЕМ аккаунте>

ВАЖНО!!! После загрузки репозитория нужно на него "переключиться" посредством команды:
