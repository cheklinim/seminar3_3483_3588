...
Для того что бы скопировать репозиторий с GitHub к себе на компьютер необходимо использовать команду:

    git clone (Адрес сылкой с сайта)

### Загрузка в репозиторий

Для того что бы отправить изменения из локального реопзитория в удаленный реопзиторий (GitHub) необходима каманда:

    git push

### Извлечение из удалёного репозитория
...
Для смены директории используется команда:

    cd (Название дериктории)
