...
Чтобы загрузить сво локальные изменения в удаленный репозиторий необходимо ввести команду:

    git push

Эта команда срабатывает только в случае, если вы клонировали с сервера, на котором у вас есть право на запись, и если никто другой с тех пор не выполнял команду push. В противном случаем команда будет откланена. Вам сперва необходимо будет получить изменения из удаленнного репозитория (git pull) и объединить их с вашими и только после этого выполнить команду git push  повторно.

### Изменение имени удаленного репозитория

Для переименования удаленного репозитория нужно выполнить команду:

    git remote rename

Например, если вы хотите переименовать pb в paul, вы можете это сделать используя эту команду.

Но, стоить помнить, что это также изменит имена удаленных веток в вашем репозитории.
