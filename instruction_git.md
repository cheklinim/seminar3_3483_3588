...
Для выгрузки на удаленный репозиторий на GitHub нужна обязательно авторизация на сервисе. При первом подключении к удаленному репозиторию нужно будет "подружить" локальный и удаленный репозитории. GitHub подскажет, как это сделать.
...
По умолчанию, после выполнения команды **git pull** всегда происходит слияние загруженных данных с локальной версией (аналогично команде git merge).